...
plt_data <- fulldata_s %>% 
  mutate(lang_groups = case_when(dominance <= -10 ~ "hi_nat",
                                 ...
                                 dominance >= 10 ~ "hi_eng")) %>% 
  group_by(condition, other_lang, lang_groups) %>% 
  summarize(meanrt = mean(meanRT, na.rm=TRUE), sdrt = sd(meanRT, na.rm = TRUE), meanscore = mean(score, na.rm = TRUE), sdscore = sd(score, na.rm = TRUE))

ggplot(plt_data) + 
  geom_point(aes(x = condition, color = condition, y = meanscore)) + 
  geom_errorbar(aes(x=condition, ymin=meanscore-sdscore, ymax=meanscore+sdscore)) +
  facet_wrap(~ lang_groups * other_lang) +
  theme_few() +
  theme(axis.title.y = element_text(margin = margin(8,8,8,8)),
        axis.title.x = element_text(margin = margin(8,8,8,8))) +
  scale_color_few()
...
anova(lm(score ~ MMI_weighted*MMI_lang*condition, data=fulldata_s))
...
ggplot(data = df, aes(x = as.factor(MMI_groups), y = mem, color = as.factor(MMI_lang_groups), group = as.factor(MMI_lang_groups))) + 
  geom_point() +
  geom_line() +
  geom_errorbar(aes(x=as.factor(MMI_groups), ymin=mem-se_mem, ymax=mem+se_mem), width = 0.05) +
